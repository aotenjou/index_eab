...
class Parser:
    ...
    def gain_candidates(self):
        for tb in self.table_info:
            ...
            if len(tb_info.conditions) > 0:
                ...
                if len(tb_info.used_columns) <= MAX_INDEX_WIDTH:
                    for _f2_idx in temp:
    ...
    def parse_a_expr(self, a_expr, is_in_or, is_target):
        ...
        etype = etype1 + etype2
        # print(a_expr)
        if not is_target and op in self.support_op.keys() and is_o1 and is_o2 and (
    ...
    def parse_update(self, update_stmt):
        ...
        # (2)parse target
        if 'targetList' in update_stmt.keys():
            target_list = update_stmt['targetList']
            self.parse_targets(target_list)

...
class QueryInfo:
    ...
    large_columns: Dict[str, Set[str]]
    exist_indexes: Set[Index]
    basic_cost_explain: float
...
class Workload:
    def __init__(
        ...
        self.basic_cost = self.basic_cost_real or self.basic_cost_explain or sum(q.basic_cost * q.frequency for q in queries)
        for q in queries:
    ...
    def indexable_columns(self):
        # 实现原Workload类的indexable_columns方法
        columns = set()
        for table, cols in self.tables_columns.items():
            for col in cols:
                # 假设Column对象有table和name属性，这里可能需要调整
...
class WorkloadGenerator(object):
    def __init__(
        ...
        self.benchmark = config["benchmark"] # 改用自己的
    ...
    def _workloads_from_tuples(self, tuples, unknown_query_probability=None):
        ...
        for tupl in tuples:
            ...
            for query_class, frequency in zip(query_classes, query_class_frequencies):
                ...
                old_query = OldQuery(query_class, query_text, frequency=frequency)
                self._store_indexable_columns(old_query)

                tables_columns = {}
                for column in old_query.columns:
                    table_name = column.table.name
                    if table_name not in tables_columns:
                        tables_columns[table_name] = set()
                    tables_columns[table_name].add(column.name)

                # 创建QueryInfo对象（假设large_columns为空，exist_indexes需要后续处理）
                query_info = QueryInfo(
                    sql=old_query.text,
                    tables_columns=tables_columns,
                    large_columns={},
                    exist_indexes=set(),
                    basic_cost_explain=0.0,
                    frequency=old_query.frequency
                )
                queries.add(query_info)

            # 创建新的Workload对象
            workload_id = self.workload_id_counter
            self.workload_id_counter += 1
            workload = Workload(
                id=workload_id,
                queries=queries,

...
class ActionManager(object):
    ...
    def get_initial_valid_actions(self, workload, budget):
        ...
        # 1 for the first(only) column in a single column index present,
        # 0.5 for the second column in a two-column index present,
        # 0.33 for the third column in a three-column index present, ...
        ...
        # single-column index: indexable columns in the workload (syntactically relevant)
        self._valid_actions_based_on_workload(workload)
        # filter the index that will exceed the budget
    ...
    def update_valid_actions(self, last_action, budget, current_storage_consumption):
        ...
        if actions_index_width == 1:
        ...
        else:  # `precond`: (A, B) will be made valid only when (A) is chosen.
            ...
            # Creating an index (A,B) drops the index (A).
...
class DRLindaActionManager(ActionManager):
    def __init__(
        ...
        # This is the same as the Experiment's object globally_indexable_columns_flat attribute
...
class DRLindaActionManagerMultiCol(ActionManager):
    def __init__(
            self, indexable_column_combinations, action_storage_consumptions,
            sb_version, max_index_width, max_index_num, reenable_indexes, constraint
    ):
    ...
    def get_initial_valid_actions(self, workload, budget):
        # 0 for actions not taken yet, 1 for single column index present
        self.current_action_status = [0 for _ in range(self.number_of_actions)]
...
class DRLindaActionManagerMultiColPos(ActionManager):
    def __init__(
            self, indexable_column_combinations, action_storage_consumptions,
            sb_version, max_index_width, max_index_num, reenable_indexes, constraint
    ):
    ...
    def update_valid_actions(self, last_action, budget, current_storage_consumption):
        ...
        if actions_index_width == 1:
            ...
            for p, last_action_column in enumerate(self.indexable_column_combinations_flat[last_action]):
                status_value = 1 / (p + 1)
                last_action_column_idx = self.column_to_idx[last_action_column]
                self.current_action_status[last_action_column_idx] += status_value
...
class DQNActionManager(ActionManager):
    def __init__(
            self, indexable_column_combinations, action_storage_consumptions,
            sb_version, max_index_width, max_index_num, reenable_indexes, constraint
    ):
        ...
        self.indexable_column_combinations = indexable_column_combinations
        # This is the same as the Experiment's object globally_index_candidates_flat attribute
        self.indexable_column_combinations_flat = [
            item for sublist in self.indexable_column_combinations for item in sublist
        ]
        self.number_of_actions = len(self.indexable_column_combinations_flat)
    ...
    def update_valid_actions(self, last_action, budget, current_storage_consumption):
        if self.indexable_column_combinations_flat[last_action] not in self.current_combinations:
            actions_index_width = len(self.indexable_column_combinations_flat[last_action])
            if actions_index_width == 1:
                self.current_action_status[last_action] = 1
            else:
...
class DQNActionManagerNonPos(ActionManager):
    def __init__(
            self, indexable_column_combinations, action_storage_consumptions,
            sb_version, max_index_width, max_index_num, reenable_indexes, constraint
    ):
        ...
        # self.number_of_actions = len(self.indexable_column_combinations_flat)
        self.number_of_actions = sum([len(combination) for combination in self.indexable_column_combinations])
        self.number_of_columns = len(self.indexable_column_combinations[0])
...
class MultiColumnIndexActionManager(ActionManager):
    def __init__(
        ...
        for column_combination_idx, indexable_column_combination in enumerate(self.indexable_column_combinations_flat):
            if len(indexable_column_combination) < 2:  # =1, continue
                continue
...
class MultiColumnIndexActionManagerNonMasking(ActionManager):
    ...
    def update_valid_actions(self, last_action, budget, current_storage_consumption):
        ...
        # mask: 1) already selected, 2) exceed the budget.
        self._valid_actions_based_on_last_action(last_action)
        self._valid_actions_based_on_budget(budget, current_storage_consumption)
        ...
        if budget is not None:
            ...
            for action_idx in self._remaining_valid_actions:
                if b_to_mb(current_storage_consumption + self.action_storage_consumptions[action_idx]) > budget:
                    ...
                    if self.valid_actions[action_idx] != self.FORBIDDEN_ACTION:
                        new_remaining_actions.append(action_idx)
...
class MultiColumnIndexActionManagerNumber(ActionManager):
    def __init__(
            self, indexable_column_combinations, action_storage_consumptions,
            sb_version, max_index_width, max_index_num, reenable_indexes
    ):
        ActionManager.__init__(self, sb_version, max_index_width=max_index_width)

...
class KfacOptimizer:
    ...
    def get_factors(self, gradients, varlist):
        ...
        def _search_factors(gradient, graph):
            ...
            if 'AddN' in bprop_op_name:
                ...
                if self.verbose > 1:
                    # : need to check all the attribute of the ops as well
                    print(gradient.name)
                    print(op_names)
        ...
        # : there may be a bug to detect associate bias and weights for forking layer, e.g. in inception models.
    ...
    def compute_stats_eigen(self):
        ...
        """
        # : figure out why this op has delays (possibly moving eigenvectors around?)
        with tf.device('/cpu:0'):
        ...
        for grad, var in zip(gradlist, varlist):
            ...
            if (len(fprop_factored_fishers) + len(bprop_factored_fishers)) > 0:
                ...
                if (self.stats[var]['assnBias'] is not None) and not self._blockdiag_bias:
                    ...
                    # : figure out how to factorize bias grad
                    # stack bias grad

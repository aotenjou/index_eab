...
class FeedForwardPolicy(DQNPolicy):
            ...
            with tf.variable_scope("action_value"):
                if feature_extraction == "cnn":
                    ...
                    for layer_size in layers:
                        ...
                        action_out = tf.layers.dense(action_out, layer_size, activation=None)
                        if layer_norm:
                            # action_out = tf_layers.layer_norm(action_out, center=True, scale=True)
                            action_out = tf_layers.LayerNormalization(center=True, scale=True)(action_out)
                        action_out = act_fun(action_out)

                # action_scores = tf_layers.fully_connected(action_out, num_outputs=self.n_actions, activation_fn=None)
                action_scores = tf.layers.dense(action_out, self.n_actions, activation=None)
                ...
                with tf.variable_scope("state_value"):
                    ...
                    for layer_size in layers:
                        ...
                        if layer_norm:
                            ...
                            state_out = tf_layers.LayerNormalization(center=True, scale=True)(state_out)
    ...
    def step(self, obs, state=None, mask=None, deterministic=True, action_mask=None):
        ...
        #     result = tape.gradient(tf.convert_to_tensor(np.argmax(q_values, axis=1)), self.obs_ph)
        # raise NotImplementedError

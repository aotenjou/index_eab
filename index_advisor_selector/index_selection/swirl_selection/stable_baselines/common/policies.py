...
def new_cost_cnn(scaled_images, **kwargs):
    """
    CNN from Nature paper.
    ...
    layer_2 = activ(conv(layer_1, 'c2', n_filters=64, filter_size=7, stride=1, init_scale=np.sqrt(2), **kwargs))
    layer_3 = activ(conv(layer_2, 'c3', n_filters=64, filter_size=7, stride=1, init_scale=np.sqrt(2), **kwargs))
    layer_3 = conv_to_fc(layer_3)
    return activ(linear(layer_3, 'fc1', n_hidden=512, init_scale=np.sqrt(2)))
...
def nature_cnn(scaled_images, **kwargs):
    ...
    layer_1 = activ(conv(scaled_images, 'c1', n_filters=32, filter_size=4, stride=1, init_scale=np.sqrt(2), **kwargs))
    layer_2 = activ(conv(layer_1, 'c2', n_filters=64, filter_size=4, stride=1, init_scale=np.sqrt(2), **kwargs))
    layer_2 = conv_to_fc(layer_2)
    return activ(linear(layer_2, 'fc1', n_hidden=512, init_scale=np.sqrt(2)))


def nature_cnn2(scaled_images, **kwargs):
    """
    CNN from Nature paper.
    ...
    print("####### USING nature_cnn2")

    layer_1 = activ(conv(scaled_images, 'c1', n_filters=64, filter_size=4, stride=1, init_scale=np.sqrt(2), **kwargs))
    layer_2 = activ(conv(layer_1, 'c2', n_filters=128, filter_size=4, stride=1, init_scale=np.sqrt(2), **kwargs))
    layer_2 = conv_to_fc(layer_2)
    return activ(linear(layer_2, 'fc1', n_hidden=1024, init_scale=np.sqrt(2)))
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, scale=False):
        super(ActorCriticPolicy, self).__init__(sess, ob_space, ac_space, n_env,
                                                n_steps, n_batch, reuse=reuse, scale=scale)
    ...
    def _setup_init(self):
        ...
        with tf.variable_scope("output", reuse=True):
            ...
            if isinstance(self.proba_distribution, CategoricalProbabilityDistribution) or isinstance(
            ...
            # self._action = self.proba_distribution.pg_sample()
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    def step(self, obs, state=None, mask=None, deterministic=False, action_mask=None):
        ...
        if deterministic:
            ...
            #     tape.watch(self.obs_ph)
            #     action, value, neglogp = self.sess.run([self.deterministic_action, self.value_flat, self.neglogp],
            #                                            feed_dict)
            #     result = tape.gradient(tf.convert_to_tensor(action), self.obs_ph)
            #
            # print(result)
            # raise NotImplementedError
            ...
            action, value, neglogp = self.sess.run([self.deterministic_action, self.value_flat, self.neglogp],
                                                   feed_dict)
            # if action[0] == 0 and action_mask[0][0] == 0:
            #     print(1)
            #     self.sess.run([self.deterministic_action, self.value_flat, self.neglogp],
            #                   feed_dict)
        else:
...
class CnnOriginalPolicy(FeedForwardPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(CnnOriginalPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
                                                cnn_extractor=original_nature_cnn,
                                                feature_extraction="cnn", **_kwargs)


class CnnNewCostPolicy(FeedForwardPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(CnnNewCostPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
                                               cnn_extractor=new_cost_cnn,
                                               feature_extraction="cnn", **_kwargs)


class CnnPolicy2(FeedForwardPolicy):
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(CnnPolicy2, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
                                         cnn_extractor=nature_cnn2,
                                         feature_extraction="cnn", **_kwargs)
...
_policy_registry = {
    ActorCriticPolicy: {
        ...
        "CnnPolicy2": CnnPolicy2,
        "CnnOriginalPolicy": CnnOriginalPolicy,
        "CnnNewCostPolicy": CnnNewCostPolicy
    }
}

...
class EvalCallbackWithTB(EventCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            # Log scalar value
            summary = tf.Summary(
                value=[tf.Summary.Value(tag='episode_reward/validation_reward', simple_value=mean_reward)])
            self.locals['writer'].add_summary(summary, self.num_timesteps)

            summary = tf.Summary(
                value=[tf.Summary.Value(tag='episode_reward/validation_length', simple_value=mean_ep_length)])
            self.locals['writer'].add_summary(summary, self.num_timesteps)
...
class EvalCallbackWithTBRunningAverage(EventCallback):
    ...
    def _on_step(self) -> bool:
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            assert len(episode_performances) == len(
                episode_rewards), f"{len(episode_performances)} vs {len(episode_rewards)}, " \
                                  f"{episode_performances} vs {episode_rewards}"
            # try:
            ...
            if moving_average is not None and moving_average > self.best_mean_ra_reward:
                if self.verbose > 0:
                    print(
                        f"New best running average reward: {moving_average} over {self.best_mean_ra_reward}\n  {self.mean_rewards}")
                if self.best_model_save_path is not None:
                    self.moving_average_step = self.n_calls
                    self.model.save(
                        os.path.join(self.best_model_save_path, f'moving_average_model{self.model_appendix}'))
            ...
            if moving_average_3 is not None and moving_average_3 > self.best_mean_ra_reward_3:
                if self.best_model_save_path is not None:
                    self.model.save(
                        os.path.join(self.best_model_save_path, f'moving_average_model_3{self.model_appendix}'))
            ...
            if mean_reward > self.best_mean_reward:
                ...
                if self.best_model_save_path is not None:
                    ...
                    self.model.save(
                        os.path.join(self.best_model_save_path, f'best_mean_reward_model{self.model_appendix}'))
                self.best_mean_reward = mean_reward

            # Log scalar value
            summary = tf.Summary(
                value=[tf.Summary.Value(tag=f'episode_reward/validation_reward_{self.name}', simple_value=mean_reward)])
            self.locals['writer'].add_summary(summary, self.num_timesteps)

            summary = tf.Summary(value=[
                tf.Summary.Value(tag=f'episode_reward/validation_achieved_percentage_{self.name}',
            ...
            summary = tf.Summary(value=[
                tf.Summary.Value(tag=f'episode_reward/validation_length_{self.name}',
